#!/usr/bin/env bash
# Optional: pre-download all systems' weights into the shared HF cache
...
echo "  docker compose logs -f worker-tortoise worker-cosyvoice2 worker-metavoice worker-xtts worker-styletts2 worker-f5tts worker-maskgct worker-fishspeech worker-ssrspeech worker-llasa"
